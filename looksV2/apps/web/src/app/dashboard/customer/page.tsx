...
import { Calendar, Clock, MapPin, Star, Heart, Ban, CheckCircle, RefreshCw, Loader2, ShieldCheck } from 'lucide-react';
import Link from 'next/link';

// Features imports
import { TestimonialsHub } from '../../../features/testimonials/TestimonialsHub';
import { AadhaarVerificationModal } from '../../../features/auth/AadhaarVerificationModal';
import { MapTracking } from '../../../features/location/MapTracking';
...
export default function CustomerDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'UPCOMING' | 'HISTORY' | 'WISHLIST' | 'FEEDBACK'>('UPCOMING');
  const [wishlist, setWishlist] = useState<any[]>([]);

  // Aadhaar states
  const [isAadhaarOpen, setIsAadhaarOpen] = useState(false);
  const [kycStatus, setKycStatus] = useState('UNVERIFIED');
  const [redactedId, setRedactedId] = useState('');
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 py-12 flex flex-col gap-10">
      ...
      {/* Profile Header */}
      <div className="bg-surface border border-borderColor rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-sm text-left">
        ...
        <div className="flex flex-col md:items-end gap-2 font-semibold text-sm">
          ...
          {/* Aadhaar status block */}
          {kycStatus === 'VERIFIED' ? (
            <span className="text-semantic-success flex items-center gap-1 text-xs font-bold bg-semantic-success/10 px-2.5 py-1 rounded-lg border border-semantic-success/20">
              <ShieldCheck size={14} /> E-KYC Verified: {redactedId}
            </span>
          ) : (
            <button
              onClick={() => setIsAadhaarOpen(true)}
              className="text-primary hover:text-primary-hover font-bold text-xs bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/20 transition-all uppercase tracking-wide"
            >
              Verify Aadhaar Identity &rarr;
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="bg-white border border-borderColor rounded-3xl p-6 shadow-sm flex flex-col gap-4 text-left">
            <div className="flex justify-between items-center border-b border-borderColor/60 pb-3">
              ...
              <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-1 rounded-md uppercase">Live Radar</span>
            </div>
          ...
          <div className="flex gap-4 border-b border-borderColor pb-3 text-sm font-bold text-muted justify-start">
            ...
            <button
              ...
              className={`pb-3 relative transition-all ${
                activeTab === 'FEEDBACK' ? 'text-primary border-b-2 border-primary' : 'hover:text-ink'
              }`}
            >
              KYC & Feedback
            </button>
          ...
          <div className="flex-grow">
            {loading ? (
              ...
              upcomingBookings.length === 0 ? (
                <div className="border border-dashed border-borderColor rounded-3xl py-20 text-center flex flex-col items-center gap-4 p-6 bg-white">
                ...
                <div className="flex flex-col gap-6">
                  {/* Real-time Map tracking when active bookings exist */}
                  <MapTracking />
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {upcomingBookings.map((b) => (
                      <div key={b.id} className="bg-white border border-borderColor rounded-2xl p-6 flex flex-col justify-between shadow-sm hover:shadow-md transition-all text-left">
              ...
              pastBookings.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {pastBookings.map((b) => (
                    <div key={b.id} className="bg-white border border-borderColor rounded-2xl p-6 flex flex-col justify-between opacity-80 hover:opacity-100 transition-all text-left">
                      <div className="flex flex-col gap-4">
                        <div className="flex justify-between items-center">
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wider ${
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {wishlist.map((srv) => (
                  <div key={srv.id} className="bg-white border border-borderColor rounded-2xl p-6 flex justify-between items-center shadow-sm text-left">
                    <div className="flex flex-col gap-1">
        ...
        {/* Right Sidebar - Security & SOS Emergency panel */}
        <div className="flex flex-col gap-6">
          <div className="bg-white border border-borderColor rounded-3xl p-6 shadow-sm flex flex-col gap-4 text-left">
            <h3 className="font-extrabold text-sm text-ink uppercase tracking-wider">Lakhnavi Safety Center</h3>
            <p className="text-xs text-muted font-medium leading-relaxed">
              Use the emergency safety trigger if you feel unsafe or need immediate assistance during your active service dispatch windows.
            </p>
            <SosButton />
          </div>

          <div className="bg-surface border border-borderColor p-5 rounded-3xl text-left text-xs font-semibold text-muted leading-relaxed">
            🛡️ <span className="text-ink font-bold">100% Client-Side Vetting</span>: All provider dispatches are tracked locally. Call logs are simulated securely.
          </div>
        </div>
      ...
      <AadhaarVerificationModal 
        isOpen={isAadhaarOpen}
        onClose={() => setIsAadhaarOpen(false)}
        onSuccess={(redacted) => {
          setKycStatus('VERIFIED');
          setRedactedId(redacted);
        }}
      />
